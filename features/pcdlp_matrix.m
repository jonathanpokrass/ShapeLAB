function [M,Norm,Neigh,Area] = pcdlp_matrix(X, tdim, opt)
%
...
[II JJ SS Norm NII NJJ NSS Area] = pcdlaplace_(X, tdim, opt);
M=sparse(II, JJ, SS);
Neigh = sparse(NII,NJJ,NSS);
% Parsing Option.
...
if ~isfield(option,'rho'),
	option = setfield(option,'rho', 1.3);
end
